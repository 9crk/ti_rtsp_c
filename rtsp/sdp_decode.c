...
/*	printf( "2nd sdplang statement in media\n");
*/
	return (-1);
      }
...
/*	printf( "2nd sdplang statement in session\n");
*/
	return (-1);
      }
  ...
  case 4: /* lang*/
    if (mptr != NULL) {
      if (mptr->lang != NULL) {
/*	printf( "2nd lang statement in media\n");
*/
	return (-1);
      }
...
/*      printf( "2nd orient type statement\n");
*/
      return (-1);
    }
...
/*	printf( "2nd control statement in media\n");
*/
	return (-1);
      }
...
/*	printf( "2nd control statement in session\n");
*/
	return (-1);
      }
...
*/
  ...
  if (lptr != NULL) {
    ...
    ADV_SPACE(lptr);
    if (!isdigit(*lptr)) {
/*      printf( "c=: garbage after multicast ttl %s\n", lptr);
*/
      free_connect_desc(cptr);
      return (ESDP_CONNECT);
    }
  ...
  while (errret == 0 && get_next_line(&line, decode, &linelen) != FALSE) {
	...
	*translated = *translated + 1;
	...
	errret = ESDP_UNKNOWN_LINE;
      ...
      printf( "More than 1 character code - %s\n", line);
      errret = ESDP_UNKNOWN_LINE;
    }
  }
