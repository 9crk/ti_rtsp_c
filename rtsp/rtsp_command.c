...
static const char *UserAgent = "YinHe Ltd.co RTSP 1.0";
...
/*     printf( "DESCRIBE returned correctly\n");
*/
...
*/
  ...
  if (ret == RTSP_RESPONSE_GOOD) {
/*   printf( "SETUP returned correctly\n");
*/
    *decode_result = info->decode_response;
    info->decode_response = NULL;
...
/*	printf( "Session for %s returned different %s %s\n",
			   ...
			   rtsp_command_t *cmd)
{
  ...
  info = session->parent;
  if (info == NULL) {
   printf( "Session doesn't point to parent\n");
    return (FALSE);
  }
  ...
  if (sptr == NULL) {
   printf( "session not found in info list\n");
    return (FALSE);
  }
  ...
  if ((cmd != NULL) &&
   ...
   printf( "Have cmd->session set wrong\n");
    return (FALSE);
  }
  return (TRUE);
}
...
*/
  ...
  if (ret == RTSP_RESPONSE_GOOD) {
/*   printf( "TEARDOWN returned correctly\n");
*/
    *decode_result = info->decode_response;
    info->decode_response = NULL;
    return (RTSP_RESPONSE_GOOD);
  } else {
   printf( "TEARDOWN return code %d\n", ret);
    if (ret != RTSP_RESPONSE_RECV_ERROR &&
	decode != NULL) {
      *decode_result = info->decode_response;
      info->decode_response = NULL;
